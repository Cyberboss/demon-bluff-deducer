...
use super::{
	DesireType, HypothesisBuilderType, testimony_exonerates::TestimonyExoneratesHypothesisBuilder,
};
use crate::{
	Breakpoint,
	engine::{
...
/// If a given [`Testimony`] [`Expression`] exonerates a give defendant ASSUMING it is true
#[derive(Debug)]
pub struct TestimonyExoneratesExpressionHypothesis {
	testifier: VillagerIndex,
	defendant: VillagerIndex,
	testimony_expression: Expression<Testimony>,
	component_exonerates_hypotheses: HashMap<Testimony, HypothesisReference>,
}
...
impl HypothesisBuilder for TestimonyExoneratesExpressionHypothesisBuilder {
	fn build(
		...
		let mut component_exonerates_hypotheses = HashMap::new();
		self.build_component_exonerates_hypotheses(
			&mut component_exonerates_hypotheses,
			registrar,
...
impl Hypothesis for TestimonyExoneratesExpressionHypothesis {
	fn describe(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
		write!(
			f,
			"Testimony expression from {} exonerates {}: {}",
		...
		FDebugBreak: FnMut(Breakpoint) + Clone,
	{
		...
		for (testimony, hypothesis_reference) in &self.component_exonerates_hypotheses {
			evaluations.insert(
				testimony.clone(),
...
impl TestimonyExoneratesExpressionHypothesisBuilder {
	...
	fn build_component_exonerates_hypotheses(
		&self,
		component_exonerates_hypotheses: &mut HashMap<Testimony, HypothesisReference>,
		registrar: &mut impl HypothesisRegistrar<HypothesisBuilderType, DesireType>,
		expression: &Expression<Testimony>,
	) {
		match expression {
			Expression::Leaf(testimony) => {
				component_exonerates_hypotheses.insert(
					testimony.clone(),
					registrar.register(TestimonyExoneratesHypothesisBuilder::new(
