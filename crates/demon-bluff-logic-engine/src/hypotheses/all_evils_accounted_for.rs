...
impl Hypothesis for AllEvilsAccountedForHypothesis {
		...
		FDebugBreak: FnMut(Breakpoint) + Clone,
	{
		...
		for index in game_state.villager_indicies() {
			let sub_hypothesis =
				evaluator.sub_evaluate(self.index_evil_hypotheses.get(&index).expect(
			...
			.combinations(game_state.total_evils() as usize)
		{
			...
			for index in combo.into_iter() {
				let index_evil_probability = villager_evil_probabilities.get(&index).expect(
					"Bro, we literally calculated this two lines ago, is villager_indicies impure??",
				);
